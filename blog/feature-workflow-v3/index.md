# Feature Workflow v3：Command + Agent + Skill 三层架构演进

作者：杨正武
来源：https://imcoders.cn/blog/feature-workflow-v3/
发布：2026-04-01T00:00:00.000Z
更新：2026-04-01T00:00:00.000Z
...
v1 时代，所有操作都在主会话中执行，AI 的开发上下文和用户的对话上下文混在一起，导致会话很快变得臃肿。v2 试图通过 Shell 脚本启动独立进程来隔离，但 `claude --print` 是非交互模式，启动后无法干预，通信只能通过文件轮询。
...
**DevSubAgent（Agent）**是执行器，拥有独立的 200k 上下文，不污染主对话。它是一个 Skill 编排器，按顺序调用已注册的 Skills 完成一个 feature 的完整生命周期。核心原则是：通过 Skill Tool 调用，不读文档，不重复实现。

### 为什么废弃 MateAgent

在最初的设计中，有一个独立的 MateAgent 作为调度器。但 Claude Code v2.1.x 有一个限制：自定义 SubAgent 不能再派生 SubAgent。而且调度逻辑本身不需要独立上下文，完全可以在 Command 的主上下文中执行。所以调度逻辑直接合并到了 dev-agent Command，架构更简洁。
...
这意味着工作流不再需要手动复制文件到 `.claude/` 目录，安装、更新、卸载都有了标准化的流程。
